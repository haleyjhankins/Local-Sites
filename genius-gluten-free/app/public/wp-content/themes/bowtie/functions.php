...
function bowtie_setup() {
	...
	register_nav_menus( array(
		...
		'primary-absolute' => esc_html__( 'Primary Menu (Absolute)', 'bowtie' ),
		'responsive' => esc_html__( 'Responsive Menu', 'bowtie' ),
		'responsive-absolute' => esc_html__( 'Responsive Menu (Absolute)', 'bowtie' ),
	) );
...
function bowtie_menu_attribute_add($atts, $item, $args) {
  // Set the menu ID
		if($args->theme_location == 'primary') {
			$atts['data-scroll'] = 'true';
			$atts['data-watch'] = $item->url;
		}

  //Return the new attribute
  return $atts;
}
add_filter( 'nav_menu_link_attributes', 'bowtie_menu_attribute_add', 10, 3 );
...
function bowtie_scripts() {
	...
	// 	get_template_directory_uri() . '/assets/dist/js/foundation.min.js',
	// 	array('jquery'), '6.3.1', true
	// );
	...
	// Add Smoothscroll to footer
	wp_enqueue_script( 'smooth-scroll',
		get_template_directory_uri() . '/assets/dist/js/smooth-scroll.min.js',
		null, '11.1.0', true
	);

	// Add Headroom to footer
	wp_enqueue_script( 'headroom',
		get_template_directory_uri() . '/assets/dist/js/headroom.min.js',
		null, '0.9.4', true
	);
...
function mb_colors_register_meta_boxes() {
    add_meta_box( 'mb-colors', 'Brand Colors', 'mb_colors_display', array('page','post','product'), 'side', 'low' );
}
...
function acf_load_bread_field_choices( $field ) {
    // reset choices
    $field['choices'] = array();
...
add_filter('acf/load_field/name=select_bread', 'acf_load_bread_field_choices');
...
function social_shortcode($atts) {
	...
	if(have_rows('social_media', 'option')):
		while(have_rows('social_media', 'option')): the_row();
			$output .= '<a href="'.get_sub_field('url').'" target="_blank" class="fa social fa-'.get_sub_field('media').'" title=""></a>';
		endwhile;
	endif;
...
function icon_shortcode($atts) {
	...
	if(isset($atts['class'])) { $class = $atts['class']; } else { $class = ''; }
	if($file) {
		$output .= '<div class="icon '.$atts['type'].'-'.$atts['id'].' '.$class.'">';
